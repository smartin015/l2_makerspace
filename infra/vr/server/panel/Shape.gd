  ...
  match shapeType:
    gamestate.SHAPE.PENCIL, gamestate.SHAPE.LINE:
      ...
      for i in range(len(points)):
        ...
        path.push_back("%s %s %s" % [ml, points[i][0], points[i][1]])
      return "<path d=\"%s\" stroke=\"#%s\" fill=\"transparent\" stroke-width=\"%s\"/>" % [path.join(" "), color.to_html(false), stroke] 
    gamestate.SHAPE.CIRCLE:
      return "<circle cx=\"%s\" cy=\"%s\" r=\"%s\" stroke=\"#%s\" fill=\"transparent\" stroke-width=\"%s\"/>" % [points[0].x, points[0].y, points[1].length(), color.to_html(false), stroke]
    gamestate.SHAPE.RECTANGLE:
      ...
      return "<rect x=\"%s\" y=\"%s\" width=\"%s\" height=\"%s\" stroke=\"#%s\" fill=\"transparent\" stroke-width=\"%s\"/>" % [xy.x, xy.y, size.x, size.y, color.to_html(false), stroke]
    _:
